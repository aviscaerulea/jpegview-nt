...
| `497fec1` | Release\|x64 ビルド最適化設定統一: WICLoader PDB 無効化、JPEGView 最適化オプション追加（StringPooling, FunctionLevelLinking, IntrinsicFunctions, OptimizeReferences, EnableCOMDATFolding）|
| `5a87d08` | mimalloc.lib を git 管理可能ディレクトリに配置: mimalloc\lib64 に配置、LibraryPath に追加してクリーンビルド対応 |
...
|---|---|---|---|
| **libjpeg-turbo** | 2.1.91 | **3.2.0** | メジャーバージョンアップ、パフォーマンス改善、TurboJPEG メモリリーク修正 |
...
| **libjxl** | 0.9-snapshot | **0.12.0** | CVE 4 件修正、デコード性能改善 |
| **libheif** | 1.19.8 | **1.23.1** | CVE 25 件超修正、unci 読み込み高速化 |
| **libde265** | 1.0.16 | **1.1.1** | CVE 6 件修正、x86 デコード約 8% 高速化 |
| **libavif** | 1.3.0 | **1.4.2** | 堅牢性修正、16bit AVIF 対応 |
| **dav1d** | 1.5.1 | **1.5.4** | マルチスレッドスケジューリング改善 |
| **LibRaw** | 0.21.1 | **0.22.2** | DNG 1.7 対応追加、TALOS 報告 6 件修正 |
...
- **GHA ランナー**: `windows-2022` に固定（`windows-latest` は VS2026 へ移行済みで、v143 の ATL と `hash_map` を欠くためビルドできない）
...
並列化対象: HQ リサイズ、LDC、ガウスフィルタ、アンシャープマスク、HQ 回転、台形補正、AVIF デコード、HEIF アルファブレンド

#### 3. 先読みバッファリング

**対象ファイル**: `JPEGProvider.cpp`

- **設定可能なバッファ数**: INI 設定 `ReadAheadBuffers`（2～8、デフォルト 5）で先読みバッファ数を動的に制御
- **双方向先読み**: 空きバッファを進行方向 75%・逆方向 25% に配分し、方向転換時の応答性を向上
...
- **キャッシュ保持**: 方向転換時も先読みキャッシュを保持（TOGGLE は例外的に全破棄して高速切り替え）
